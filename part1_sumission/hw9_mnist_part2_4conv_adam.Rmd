...
summaries_dir <- '/Users/markberman/cs498_AML/assignment9/MNIST_Part2_4conv_adam/mnist_logs'
...
```{r}
with(tf$name_scope("conv4_activation"), {
  W_conv4 <- weight_variable(shape = shape(5L, 5L, 8L, 8L))
  b_conv4 <- bias_variable(shape = shape(8L))
...
with(tf$name_scope("flatten_pool2_activation"), {
  ...
  h_fc1_flat <- tf$reshape(h_conv4, shape(-1L, 28L * 28L * 8L))
